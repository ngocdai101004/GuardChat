"""CLI: evaluate the BiLSTM Task 1 baseline on GuardChat.

Output schema is the same as SafeGuider's ``eval_recognition.py`` so a
shared benchmark aggregator (e.g. ``src/benchmark/``) can compose the
final Table 1 / Table 2 summaries directly.
...
from src.BiLSTM.recognition import RecognitionPipeline  # noqa: E402


DEFAULT_WEIGHTS = os.path.join(_HERE, "weights", "bilstm_multilabel.pt")
...
def main() -> int:
    p = argparse.ArgumentParser(description="Evaluate BiLSTM Task 1 on GuardChat.")
